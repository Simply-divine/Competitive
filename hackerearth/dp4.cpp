...
ll pre_a[MAXN],pre_b[MAXN];
void sum_upto(ll &left_sum,ll l,int parity){
			left_sum = (l > 0)? ((parity==0)?pre_b[l-1]:pre_a[l-1]) : 0;
}
int main()
{
	...
	rep(i,0,n-1)cin>>b[i];
	rep(i,0,n-1){
		pre_a[i] = (i > 0)? a[i] + pre_b[i-1]: a[i];
		pre_b[i] = (i > 0)? pre_a[i-1] + b[i]: b[i];
	}
	while(q-->0){
		ll p1,l,r;
		cin>>p1>>l>>r;
		...
		if (p1 == 1){
			sum_upto(left_sum,l,0);
			if( (r - l) % 2 == 0)
			sum_upto(right_sum,r+1,1);
			else sum_upto(right_sum,r+1,0);
			cout << right_sum - left_sum << nl; 
		}
		else{
			sum_upto(left_sum,l,1);
			if((r-l)%2==0)
			sum_upto(right_sum,r+1,0);
			else sum_upto(right_sum,r+1,1);
			cout<< right_sum - left_sum << nl;			
		}	
	}	
	return 0;
