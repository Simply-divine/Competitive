...
struct node *lca(ll x,ll y,struct node *root){
	...
	int boy1[MAXN+1][MAXN+1],boy2[MAXN+1][MAXN+1],girl1[MAXN+1][MAXN+1],girl2[MAXN+1][MAXN+1];
int main()
{
	...
	rep(i,1,n) rep(j,1,m) boy1[i][j] = max(boy1[i-1][j],boy1[i][j-1]) + mat[i][j];
	rrep(i,n,1) rrep(j,m,1) boy2[i][j] = max(boy2[i+1][j],boy2[i][j+1]) + mat[i][j];
	rep(i,1,n) rep(j,1,m) girl1[i][j] = max(girl1[i+1][j],girl1[i][j-1]) + mat[i][j];
	rrep(i,n,1) rrep(j,m,1) girl2[i][j] = max(girl2[i-1][j],girl2[i][j+1]) + mat[i][j];
	int ans = 0;
	rep(i,1,n)rep(j,1,m){
		int option1 = boy1[i-1][j] + boy2[i+1][j] + girl1[i][j-1] + girl2[i][j+1];
		int option2 = boy1[i][j-1] + boy2[i][j+1] + girl1[i-1][j] + girl2[i+1][j];
