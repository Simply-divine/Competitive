...
int solve(int k){
	k |= k>>1;//first two bit set
	k |= k>>2;//first four bit set
	k |= k>>4;// first sixteen bit set
	k |= k>>8;
	k |= k>>16;
	k = k+1;
	assert((k&(k-1))==0);
...
int main()
{
	...
	int k;
	cin>>k;
	int greaterBit = solve(k);
	assert((greaterBit&(greaterBit-1))==0);
	int ans[2][3];
	ans[0][0] = greaterBit + k;
	ans[0][1] = greaterBit;
	ans[1][0] = k;
	ans[1][1] = k+greaterBit;
	ans[1][2] = k;
	ans[0][2] = 0;
	cout<<"2"<<" "<< "3"<<nl;
	for(int i = 0; i < 2; i++){
