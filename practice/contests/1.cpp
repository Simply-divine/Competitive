...
string findLongestPrefixPal(string s){
	// prefix of s + "#" + s'
	string temp = s;
...
string solve(string s){
	...
	string pref = s.substr(i, n - 2* i);
	string ans1 = findLongestPrefixPal(pref);
	assert(ispal(ans1));
	reverse(pref.begin(),pref.end());
	string ans2 = findLongestPrefixPal(pref);
	assert(ispal(ans2));
	if ( ans2.size() > ans1.size()) swap(ans1,ans2);
	string temp = init;
	reverse(temp.begin(),temp.end());
	return init + ans1 + temp;	
}
int main()
{
