...
// college: SVNIT 
// slogan : I am the ocean of peace. 
#include<bits/stdc++.h>
...
struct node * newNode(ll element){
...
ll maxInPath(struct node *root, ll x){
	if(root->data < x) return maxInPath(root->right,x);
	else if ( root->data > x) return max(maxInPath(root->left,x),root->data);
	else return root->data;
}
ll find_max(int x,int y,struct node *root){
	struct node * LCA = lca(x,y,root);
	ll ans = max(maxInPath(LCA,x),maxInPath(LCA,y));
	return ans;
}
int main()
{
	...
	rep(i,0,n-1){
		ll temp;
		cin>>temp;
		if(i==0)root = newNode(temp);
		else insert(root,temp);
	}
	ll x,y;
	cin>>x>>y;
	if(x > y) swap(x,y);
	ll val = find_max(x,y,root);
	cout<<val<<nl;
	return 0;
}
