...
int main()
{
	...
	int a[n];
	rep(i,0,n-1)cin>>a[i];
	//dp[i][j][k] the number of sequence of i elements with sum j with k = 0 no L no R k = 1 l but no r k=2 r but no l and k = 3 both l and r
	rep(i,s,S)rep(j,0,3)dp[0][s][k] = 0;
	rep(i,1,n){
		rep(j,s,S) {
			// k = 0
		}
