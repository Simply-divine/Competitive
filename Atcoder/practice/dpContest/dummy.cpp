...
int main()
{
	...
	// find the max freq
	//done
	// find the first index of max freq
	//done
	int start = -1;
	rep(i,0,n-1){
		if(freq[a[i]] == mx){ start = i; break;}
	}
	// all to the left of first push them in result
	vector<pair<int,pair<int,int>>> res;
	
	// start from the start and move to the left
	for(int i = start - 1; i >= 0; --i){
		if ( a[i] < a[start]) res.push_back({1,{i,i+1}});
		else if ( a[i] > a[start]) res.push_back({2,{i,i+1}});
	}
	// from start to end 
