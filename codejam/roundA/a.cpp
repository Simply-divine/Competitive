...
int main()
{
	...
	for(int j = 0; j < t; j++){
		...
		// take vector of vector of strings seperated by *
		bool start[n] ;
		bool end[n];
		memset(end,1,n);
		memset(start,1,n);
		rep(i,0,n-1){
			string temp;
			cin>>temp;
			// if start occurs in starting dont process it
			if(temp[0] == '*'){
				start[i] = 0;
			}
			if(temp[temp.size()-1] == '*') {
		...
		//}
		//done
		// solve for first and last case by sorting them according to length
		vector<string> startcheck(n);
		rep(i,0,n-1){
			if(start[i]){
				startcheck.push_back(*s[i].begin());
			}
		}
		bool f = 0;
		sort(startcheck.begin(),startcheck.end(),comp1);
		for(int i = 1; i < startcheck.size(); i++){
			int lp = startcheck[i-1].size();
			int ln = startcheck[i].size();
			if(startcheck[i].substr(0,lp)!=startcheck[i-1]){
				f = 1;		
			}
		}
		string ans_l = startcheck[startcheck.size()-1];
		//debug(ans_l);
		//rep(i,0,n-1){
		//	for(auto x:s[i]){
		//		debug(x);
		...
		//}
		//done	
		vector<string> endcheck(n);
		rep(i,0,n-1){
			if(end[i]){
				int l = s[i].size();
				debug(*(s[i].end()-1));
				endcheck.push_back(*(s[i].end()-1));
			}
		}
		sort(endcheck.begin(),endcheck.end(),comp1);
		for(int i = 1; i < endcheck.size(); i++){
			int lp = endcheck[i-1].size();
			int ln = endcheck[i].size();
			if(endcheck[i].substr(ln - lp,lp)!=startcheck[i-1]){
				f = 1;		
			}
		}
		string ans_r = endcheck[endcheck.size()-1];
		// solve for rest by just taking all of them and make ans
		cout<<"Case #"<<(j+1)<<": ";
		cout<<ans_l + ans_r<<nl;
	}
	return 0;
